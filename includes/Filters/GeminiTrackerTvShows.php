...
class GeminiTrackerTvShows {
    ...
    public function process_tv_show($tvshow) {
        ...
        if ($response === null || count($response) === 0 || !isset($response['data']) || count($response['data']) === 0) {
            do_action('alli1d_log', 'Gemini Tracker API - No response', Logs::DEBUG, Logs::SERIES_LOG);
            return $tvshow;
        }
        do_action('alli1d_log', 'Gemini Tracker API - ' .count($response['data']). ' results', Logs::DEBUG, Logs::SERIES_LOG);
        ...
        if (null !== $file_content ) {
            ...
            do_action('alli1d_log', 'Gemini Tracker API - Torrent found : ' . $file_name, Logs::DEBUG, Logs::SERIES_LOG);
        } else {
            do_action('alli1d_log', 'Gemini Tracker API - Failed to download torrent', Logs::ERROR, Logs::SERIES_LOG);
        }
        return $tvshow;
    }
}
